...
# First need to sort list by chronological order
data = data.split("\n")
parsed_data = []
for line in data:
    line = line.split(" ")
    date = "".join(line[0][1:].split("-"))
    time = "".join(line[1][:len(line[1])-1].split(":"))
    msg = " ".join(line[2:])
    parsed_data.append((date, time, msg))
parsed_data.sort(key=lambda x: x[0]+x[1])

# Now need to iterate through parsed and sorted data
# Will store sleep/wakeup data in guard_times
guard_times = []
time_data = [0 for x in range(60)]
guard = -1
sleep_index = 0
for entry in parsed_data:
    if entry[2][0] == "G":
        # Save last guard data
        guard_times.append((guard, time_data))
        # Reinitialize all variables
        time_data = [0 for x in range(60)]
        guard = int(entry[2].split(" ")[1][1:])
    elif entry[2][0] == "f":
        sleep_index = int(entry[1][2:])
    else:
        new_index = int(entry[1][2:])
        for i in range(sleep_index, new_index):
            time_data[i] = 1

# First entry in guard_times is invalid, remove it
guard_times = guard_times[1:]
# Now, make a consolidated list from all guard_times by: Summing the lists piecewise, summing that list per guard
guard_times.sort(key=lambda x: x[0])
# final_data will have format [[guard_num, [minutes spent asleep], total_mins_asleep]]
final_data = []
guard = -1
for shift in guard_times:
    if shift[0] == guard:
        # Add the time data component-wise
        final_data[len(final_data)-1][1] = [sum(x) for x in zip(final_data[len(final_data)-1][1], shift[1])]
    else:
        # If final data isn't empty
        if final_data:
            # Calculate the total
            final_data[len(final_data)-1][2] = sum(final_data[len(final_data)-1][1])
        # Start a new entry
        guard = shift[0]
        final_data.append([shift[0], shift[1], 0])
# Need to sum the guard
final_data[len(final_data)-1][2] = sum(final_data[len(final_data)-1][1])

# Need to sort guards by the one with the highest # of times asleep on any given minute
final_data.sort(key=lambda x: max(x[1]), reverse=True)
max_min = final_data[0][1].index(max(final_data[0][1]))
max_guard = final_data[0][0]
